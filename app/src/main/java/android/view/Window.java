...
public abstract class Window {
	 ...
	 * {@link android.app.ActivityOptions#makeSceneTransitionAnimation(android.app.Activity, * android.util.Pair[])} or {@link android.app.ActivityOptions#makeSceneTransitionAnimation(
	...
	void adjustLayoutParamsForSubWindow(WindowManager.LayoutParams wp) {
		...
		if (wp.type >= WindowManager.LayoutParams.FIRST_SUB_WINDOW && wp.type <= WindowManager.LayoutParams.LAST_SUB_WINDOW) {
			...
			//应用窗口（dialog的情况）
			//此时的wp.token即为mAppToken，而这个mAppToken就是我们在Activity的attach()方法中传入的mToken!
			if (wp.token == null) {
	...
	public void setDisableWallpaperTouchEvents(boolean disable) {
		setPrivateFlags(disable ? WindowManager.LayoutParams.PRIVATE_FLAG_DISABLE_WALLPAPER_TOUCH_EVENTS : 0, WindowManager.LayoutParams.PRIVATE_FLAG_DISABLE_WALLPAPER_TOUCH_EVENTS);
	}
