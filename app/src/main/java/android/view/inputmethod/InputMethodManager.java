...
public final class InputMethodManager {
	 ...
	 */
	private int mNextUserActionNotificationSequenceNumber = NOT_AN_ACTION_NOTIFICATION_SEQUENCE_NUMBER;

	/**
	 * The last sequence number that is already sent to {@link InputMethodManagerService}.
	 */
	private int mLastSentUserActionNotificationSequenceNumber = NOT_AN_ACTION_NOTIFICATION_SEQUENCE_NUMBER;
	...
	final IInputMethodClient.Stub mClient = new IInputMethodClient.Stub() {
		...
		public void setUserActionNotificationSequenceNumber(int sequenceNumber) {
			mH.sendMessage(mH.obtainMessage(MSG_SET_USER_ACTION_NOTIFICATION_SEQUENCE_NUMBER, sequenceNumber, 0));
		}
	};
	...
	void finishedInputEvent(int seq, boolean handled, boolean timeout) {
		...
		synchronized (mH) {
			...
			if (timeout) {
				Log.w(TAG, "Timeout waiting for IME to handle input event after " + INPUT_METHOD_NOT_RESPONDING_TIMEOUT + " ms: " + p.mInputMethodId);
			} else {
