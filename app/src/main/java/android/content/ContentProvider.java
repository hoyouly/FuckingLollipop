...
public abstract class ContentProvider implements ComponentCallbacks2 {
	...
	/**
	 * @hide Implementation when a caller has performed a query on the content
	 ...
	 * and calls through to {@link #query(Uri, String[], String, String[], * String, CancellationSignal)} with that.
	 */
	...
	/**
	 * @hide Implementation when a caller has performed an insert on the content
	 * provider, but that call has been rejected for the operation given
	 ...
	 * error, you can use {@link ParcelFileDescriptor#open(File, int, * android.os.Handler, ParcelFileDescriptor.OnCloseListener)},
	...
	public interface PipeDataWriter<T> {
		 ...
		 */
		public void writeDataToPipe(ParcelFileDescriptor output, Uri uri, String mimeType, Bundle opts, T args);
	}
	...
	public static Uri maybeAddUserId(Uri uri, int userId) {
		if (uri == null) return null;
		if (userId != UserHandle.USER_CURRENT && ContentResolver.SCHEME_CONTENT.equals(uri.getScheme())) {
			if (!uriHasUserId(uri)) {
