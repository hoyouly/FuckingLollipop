...
public class Dialog implements DialogInterface, Window.Callback, KeyEvent.Callback, OnCreateContextMenuListener, Window.OnWindowDismissedCallback {
	...
	Dialog(Context context, int theme, boolean createContextThemeWrapper) {
		//这里的context一般是指Activity的，如果传递的不是Activity，在显示Dialog时会抛出异常
		...
		//这里有一点要注意的是，Dialog调用setWindowManager时，没有传递token,回到前面有关token的描述，PhoneWindow的mAppToken是在这个方法被调用时赋值的，由此说来Dialog的mAppToken是空的。这是跟应用窗口的差别所在。
		//显然 token 的值需要走默认逻辑来赋值。而默认逻辑跟 WindowManager 实例中的 mParentWindow 有很大的关系的。显然 Dialog 中的 mWindowManager 的 mParentWindow 不能为空，否则 Dialog 的 token就没有赋值，创建窗口时会抛异常的。
	 ...
	 * @hide
	 */
	public boolean takeCancelAndDismissListeners(String msg, final OnCancelListener cancel, final OnDismissListener dismiss) {
		if (mCancelAndDismissTaken != null) {
