...
public class InputMethodManagerService extends IInputMethodManager.Stub implements ServiceConnection, Handler.Callback {
	...
	private final LruCache<SuggestionSpan, InputMethodInfo> mSecureSuggestionSpans = new LruCache<SuggestionSpan, InputMethodInfo>(SECURE_SUGGESTION_SPANS_MAX_SIZE);
	...
	final HashMap<IBinder, ClientState> mClients = new HashMap<IBinder, ClientState>();
	...
	private final HashMap<InputMethodInfo, ArrayList<InputMethodSubtype>> mShortcutInputMethodsAndSubtypes = new HashMap<InputMethodInfo, ArrayList<InputMethodSubtype>>();
	...
	class SettingsObserver extends ContentObserver {
		...
		SettingsObserver(Handler handler) {
			...
			resolver.registerContentObserver(Settings.Secure.getUriFor(Settings.Secure.SHOW_IME_WITH_HARD_KEYBOARD), false, this);
		}
	...
	class MyPackageMonitor extends PackageMonitor {
		...
		public void onSomePackagesChanged() {
			...
			synchronized (mMethodMap) {
				...
				buildInputMethodListLocked(mMethodList, mMethodMap, false /* resetDefaultEnabledIme */);
	...
	private class HardKeyboardListener implements WindowManagerService.OnHardKeyboardStatusChangeListener {
		...
		public void onHardKeyboardStatusChange(boolean available) {
			mHandler.sendMessage(mHandler.obtainMessage(MSG_HARD_KEYBOARD_SWITCH_CHANGED, available ? 1 : 0));
		}
	...
	public InputMethodManagerService(Context context, WindowManagerService windowManager) {
		...
		synchronized (mMethodMap) {
			buildInputMethodListLocked(mMethodList, mMethodMap, !mImeSelectedOnBoot /* resetDefaultEnabledIme */);
		}
	...
	private void resetStateIfCurrentLocaleChangedLocked() {
		resetAllInternalStateLocked(true /* updateOnlyWhenLocaleChanged */, true /* resetDefaultImeLocked */);
	}
	...
	private void switchUserLocked(int newUserId) {
		...
		resetAllInternalStateLocked(false  /* updateOnlyWhenLocaleChanged */, initialUserSwitch /* needsToResetDefaultIme */);
	...
	void updateCurrentProfileIds() {
		List<UserInfo> profiles = UserManager.get(mContext).getProfiles(mSettings.getCurrentUserId());
	...
	InputBindResult attachNewInputLocked(boolean initial) {
		...
		if (initial) {
			...
			executeOrSendMessage(session.method, mCaller.obtainMessageOOO(MSG_RESTART_INPUT, session, mCurInputContext, mCurAttribute));
		}
		if (mShowRequested) {
	...
	InputBindResult startInputUncheckedLocked(ClientState cs, IInputContext inputContext, EditorInfo attribute, int controlFlags) {
		...
		if (mCurClient != cs) {
			...
			// If the client is changing, we need to switch over to the new one.
			//取消当前活动程序和输入法的绑定
		...
		try {
			synchronized (mMethodMap) {
				...
				if (iconId == 0) {
					...
					if (mStatusBar != null) {
						mStatusBar.setIcon("ime", packageName, iconId, 0, contentDescription != null ? contentDescription.toString() : null);
		...
		try {
			...
			synchronized (mMethodMap) {
				...
				if (imi != null && needsToShowImeSwitcher) {
					...
					mImeSwitcherNotification.color = mContext.getResources().getColor(com.android.internal.R.color.system_notification_accent_color);
	...
	public void showInputMethodAndSubtypeEnablerFromClient(IInputMethodClient client, String inputMethodId) {
		...
		synchronized (mMethodMap) {
			...
			executeOrSendMessage(mCurMethod, mCaller.obtainMessageO(MSG_SHOW_IM_SUBTYPE_ENABLER, inputMethodId));
		}
	}
	...
	public boolean switchToNextInputMethod(IBinder token, boolean onlyCurrentIme) {
		...
		synchronized (mMethodMap) {
			...
			final ImeSubtypeListItem nextSubtype = mSwitchingController.getNextInputMethodLocked(onlyCurrentIme, mMethodMap.get(mCurMethodId), mCurrentSubtype);
			if (nextSubtype == null) {
				return false;
			}
	...
	public boolean shouldOfferSwitchingToNextInputMethod(IBinder token) {
		...
		synchronized (mMethodMap) {
			...
			final ImeSubtypeListItem nextSubtype = mSwitchingController.getNextInputMethodLocked(false /* onlyCurrentIme */, mMethodMap.get(mCurMethodId), mCurrentSubtype);
			if (nextSubtype == null) {
				return false;
			}
			return true;
	...
	private void showInputMethodMenuInternal(boolean showSubtypes) {
		...
		synchronized (mMethodMap) {
			...
			final LayoutInflater inflater = (LayoutInflater) dialogContext.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
	...
	private static class InputMethodFileManager {
		...
		private final HashMap<String, List<InputMethodSubtype>> mAdditionalSubtypesMap = new HashMap<String, List<InputMethodSubtype>>();
		...
		public InputMethodFileManager(HashMap<String, InputMethodInfo> methodMap, int userId) {
			...
			final File systemDir = userId == UserHandle.USER_OWNER ? new File(Environment.getDataDirectory(), SYSTEM_PATH) : Environment.getUserSystemDirectory(userId);
