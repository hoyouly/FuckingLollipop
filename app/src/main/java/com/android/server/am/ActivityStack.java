...
final class ActivityStack {
	...
	static final int RELEASE_BACKGROUND_RESOURCES_TIMEOUT_MSG = ActivityManagerService.FIRST_ACTIVITY_STACK_MSG + 7;
	...
	final boolean isOnHomeDisplay() {
		return isAttached() && mActivityContainer.mActivityDisplay.mDisplayId == Display.DEFAULT_DISPLAY;
	}
	...
	ActivityRecord findTaskLocked(ActivityRecord target) {
		...
		for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
			...
			if (DEBUG_TASKS)
				Slog.d(TAG, "Comparing existing cls=" + taskIntent.getComponent().flattenToShortString() + "/aff=" + r.task.rootAffinity + " to new cls=" + intent.getComponent().flattenToShortString() + "/aff=" + info.taskAffinity);
	 ...
	 *  开始暂停当前resume 的Activit 。 如果已经有一个activity 暂停或没有resume Activity，则调用此方法是错误的。
	...
	final boolean startPausingLocked(boolean userLeaving, boolean uiSleeping, boolean resuming, boolean dontWait) {
		...
		if (prev.app != null && prev.app.thread != null) {
			...
			try {
				...
				//prev.app.thread  其实就是ApplicationThread
		...
		for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
			...
			for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
				...
				if (!behindFullscreen || r.mLaunchTaskBehind) {
					...
					if (DEBUG_VISBILITY)
						Slog.v(TAG, "Make invisible? " + r + " finishing=" + r.finishing + " state=" + r.state + " behindFullscreen=" + behindFullscreen);
	...
	final boolean resumeTopActivityInnerLocked(ActivityRecord prev, Bundle options) {
		...
		// Find the first activity that is not finishing.找到第一个没有finish 的Activity。
		...
		if (next == null) {
			...
			return isOnHomeDisplay() && mStackSupervisor.resumeHomeStackTask(returnTaskType, prev, "noMoreActivities");
		}
		...
		//好的，我们现在要开始切换到“下一个”。 我们可能首先必须暂停当前的活动，但这是我们决定前往“下一个”的重要一点，所以要记住这一点。 XXX“应用重定向”对话框在这一点上误报太多，所以现在关闭。
		if (false) {
		...
		if (prev != null && prev != next) {
			if (!prev.waitingVisible && next != null && !next.nowVisible) {
				...
				if (prev.finishing) {
					...
					if (DEBUG_SWITCH)
						Slog.v(TAG, "Not waiting for visible to hide: " + prev + ", waitingVisible=" + (prev != null ? prev.waitingVisible : null) + ", nowVisible=" + next.nowVisible);
				} else {
					if (DEBUG_SWITCH)
						Slog.v(TAG, "Previous already visible but still waiting to hide: " + prev + ", waitingVisible=" + (prev != null ? prev.waitingVisible : null) + ", nowVisible=" + next.nowVisible);
				}
			}
		}
		...
		if (prev != null) {
			if (prev.finishing) {
				...
				if (mNoAnimActivities.contains(next)) {
					...
					mWindowManager.prepareAppTransition(prev.task == next.task ? AppTransition.TRANSIT_ACTIVITY_OPEN : next.mLaunchTaskBehind ? AppTransition.TRANSIT_TASK_OPEN_BEHIND : AppTransition.TRANSIT_TASK_OPEN, false);
				}
			}
			if (false) {
				mWindowManager.setAppWillBeHidden(prev.appToken);
		...
		if (next.app != null && next.app.thread != null) {
			...
			if (notUpdated) {
				...
				if (DEBUG_SWITCH || DEBUG_STATES) Slog.i(TAG, "Activity config changed during resume: " + next + ", new next: " + nextNext);
				if (nextNext != next) {
					// Do over!
		...
		for (int i = numActivities - 1; i > rootActivityNdx; --i) {
			...
			if (!finishOnTaskLaunch && !clearWhenTaskReset && target.resultTo != null) {
				...
				if (bottom != null && target.taskAffinity != null && target.taskAffinity.equals(bottom.task.affinity)) {
					...
					if (DEBUG_TASKS) Slog.v(TAG, "Start pushing activity " + target + " out to bottom task " + bottom.task);
				} else {
					targetTask = createTaskRecord(mStackSupervisor.getNextTaskId(), target.info, null, null, null, false);
				...
				for (int srcPos = start; srcPos >= i; --srcPos) {
					...
					if (DEBUG_ADD_REMOVE)
						Slog.i(TAG, "Removing activity " + p + " from task=" + task + " adding to task=" + targetTask + " Callers=" + Debug.getCallers(4));
				...
				for (int srcPos = i; srcPos <= end; srcPos++) {
					...
					if (DEBUG_TASKS) Slog.w(TAG, "resetTaskIntendedTask: calling finishActivity on " + p);
		...
		for (int i = numActivities - 1; i > rootActivityNdx; --i) {
			...
			if (target.resultTo != null) {
				...
				if (forceReset || finishOnTaskLaunch) {
					...
					final int start = replyChainEnd >= 0 ? replyChainEnd : i;
					if (DEBUG_TASKS)
						Slog.v(TAG, "Reparenting from task=" + affinityTask + ":" + start + "-" + i + " to task=" + task + ":" + taskInsertionPoint);
					for (int srcPos = start; srcPos >= i; --srcPos) {
						final ActivityRecord p = activities.get(srcPos);
						p.setTask(task, null);
						...
						if (DEBUG_ADD_REMOVE)
							Slog.i(TAG, "Removing and adding activity " + p + " to stack at " + task, new RuntimeException("here").fillInStackTrace());
		...
		if (mStackSupervisor.isFrontStack(this) && mService.mFocusedActivity == r) {
			...
			if (next != r) {
				...
				if (r.frontOfTask && task == topTask() && task.isOverHomeStack()) {
					mStackSupervisor.moveHomeStackTaskToTop(task.getTaskToReturnTo(), reason + " adjustFocus");
				}
			}
		...
		if ((r.intent.getFlags() & Intent.FLAG_ACTIVITY_NO_HISTORY) != 0 || (r.info.flags & ActivityInfo.FLAG_NO_HISTORY) != 0) {
			if (!r.finishing) {
				if (!mService.isSleeping()) {
					...
					if (DEBUG_STATES) Slog.d(TAG, "Not finishing noHistory " + r + " on stop because we're just sleeping");
				}
			}
		}
	...
	final boolean requestFinishActivityLocked(IBinder token, int resultCode, Intent resultData, String reason, boolean oomAdj) {
		...
		if (DEBUG_RESULTS || DEBUG_STATES)
			Slog.v(TAG, "Finishing activity token=" + token + " r=" + ", result=" + resultCode + ", data=" + resultData + ", reason=" + reason);
		if (r == null) {
			return false;
		}
	...
	final void finishTopRunningActivityLocked(ProcessRecord app) {
		...
		if (r != null && r.app == app) {
			...
			Slog.w(TAG, "  Force finishing activity 1 " + r.intent.getComponent().flattenToShortString());
	...
	final void finishActivityResultsLocked(ActivityRecord r, int resultCode, Intent resultData) {
		...
		if (resultTo != null) {
			if (DEBUG_RESULTS)
				Slog.v(TAG, "Adding result to " + resultTo + " who=" + r.resultWho + " req=" + r.requestCode + " res=" + resultCode + " data=" + resultData);
	...
	final boolean finishActivityLocked(ActivityRecord r, int resultCode, Intent resultData, String reason, boolean oomAdj) {
		...
		if (mResumedActivity == r) {
			...
			if (DEBUG_VISBILITY || DEBUG_TRANSITION) Slog.v(TAG, "Prepare close transition: finishing " + r);
		...
		if (parent != null && foundParentInTask) {
			...
			if (parentLaunchMode == ActivityInfo.LAUNCH_SINGLE_INSTANCE || parentLaunchMode == ActivityInfo.LAUNCH_SINGLE_TASK || parentLaunchMode == ActivityInfo.LAUNCH_SINGLE_TOP || (destIntentFlags & Intent.FLAG_ACTIVITY_CLEAR_TOP) != 0) {
				...
				try {
					...
					int res = mStackSupervisor.startActivityLocked(srec.app.thread, destIntent, null, aInfo, null, null, parent.appToken, null, 0, -1, parent.launchedFromUid, parent.launchedFromPackage, -1, parent.launchedFromUid, 0, null, true, null, null, null);
	...
	private void removeActivityFromHistoryLocked(ActivityRecord r, String reason) {
		...
		if (task != null && task.removeActivity(r)) {
			...
			if (mStackSupervisor.isFrontStack(this) && task == topTask() && task.isOverHomeStack()) {
	...
	final void destroyActivitiesLocked(ProcessRecord owner, String reason) {
		...
		for (int taskNdx = mTaskHistory.size() - 1; taskNdx >= 0; --taskNdx) {
			...
			for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
				...
				if (r.isDestroyable()) {
					if (DEBUG_SWITCH)
						Slog.v(TAG, "Destroying " + r + " in state " + r.state + " resumed=" + mResumedActivity + " pausing=" + mPausingActivity + " for reason " + reason);
		...
		for (int taskNdx = 0; taskNdx < mTaskHistory.size() && maxTasks > 0; taskNdx++) {
			...
			for (int actNdx = 0; actNdx < activities.size(); actNdx++) {
				...
				if (activity.app == app && activity.isDestroyable()) {
					if (DEBUG_RELEASE)
						Slog.v(TAG, "Destroying " + activity + " in state " + activity.state + " resumed=" + mResumedActivity + " pausing=" + mPausingActivity + " for reason " + reason);
	...
	final boolean destroyActivityLocked(ActivityRecord r, boolean removeFromApp, String reason) {
		...
		if (hadApp) {
			...
			if (r.finishing && !skipDestroy) {
				if (DEBUG_STATES) Slog.v(TAG, "Moving to DESTROYING: " + r + " (destroy requested)");
	...
	void releaseBackgroundResources() {
		if (hasVisibleBehindActivity() && !mHandler.hasMessages(RELEASE_BACKGROUND_RESOURCES_TIMEOUT_MSG)) {
			...
			if (DEBUG_STATES)
				Slog.d(TAG, "releaseBackgroundResources activtyDisplay=" + mActivityContainer.mActivityDisplay + " visibleBehind=" + r + " app=" + r.app + " thread=" + r.app.thread);
	...
	private void removeHistoryRecordsForAppLocked(ArrayList<ActivityRecord> list, ProcessRecord app, String listName) {
		int i = list.size();
		if (DEBUG_CLEANUP) Slog.v(TAG, "Removing app " + app + " from list " + listName + " with " + i + " entries");
		while (i > 0) {
			i--;
			...
			for (int activityNdx = activities.size() - 1; activityNdx >= 0; --activityNdx) {
				...
				if (r.app == app) {
					...
					if (remove) {
						...
						if (DEBUG_APP) Slog.v(TAG, "Clearing app during removeHistory for activity " + r);
						r.app = null;
						r.nowVisible = false;
						if (!r.haveState) {
							if (DEBUG_SAVED_STATE) Slog.i(TAG, "App died, clearing saved state of " + r);
							r.icicle = null;
						}
					}
	...
	void moveHomeStackTaskToTop(int homeStackTaskType) {
		...
		for (int taskNdx = top; taskNdx >= 0; --taskNdx) {
			...
			if (task.taskType == homeStackTaskType) {
				if (DEBUG_TASKS || DEBUG_STACK) Slog.d(TAG, "moveHomeStackTaskToTop: moving " + task);
	...
	final void moveTaskToFrontLocked(TaskRecord tr, ActivityRecord source, Bundle options, String reason) {
		...
		if (numTasks == 0 || index < 0) {
			// nothing to do!
			if (source != null && (source.intent.getFlags() & Intent.FLAG_ACTIVITY_NO_ANIMATION) != 0) {
				ActivityOptions.abort(options);
			} else {
	...
	public void unhandledBackLocked() {
		final int top = mTaskHistory.size() - 1;
		if (DEBUG_SWITCH) Slog.d(TAG, "Performing unhandledBack(): top activity at " + top);
		if (top >= 0) {
			final ArrayList<ActivityRecord> activities = mTaskHistory.get(top).mActivities;
			int activityTop = activities.size() - 1;
			if (activityTop > 0) {
				finishActivityLocked(activities.get(activityTop), Activity.RESULT_CANCELED, null, "unhandled-back", true);
			}
		}
	}
	...
	public String toString() {
		return "ActivityStack{" + Integer.toHexString(System.identityHashCode(this)) + " stackId=" + mStackId + ", " + mTaskHistory.size() + " tasks}";
	}
}
