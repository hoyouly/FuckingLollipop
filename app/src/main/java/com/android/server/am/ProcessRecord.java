...
final class ProcessRecord {
	...
	final ArrayMap<String, ProcessStats.ProcessStateHolder> pkgList = new ArrayMap<String, ProcessStats.ProcessStateHolder>();
	...
	public void makeActive(IApplicationThread _thread, ProcessStatsService tracker) {
		if (thread == null) {
			...
			for (int i = 0; i < pkgList.size(); i++) {
				...
				holder.state = tracker.getProcessStateLocked(pkgList.keyAt(i), info.uid, info.versionCode, processName);
	...
	void kill(String reason, boolean noisy) {
		if (!killedByAm) {
			if (noisy) {
				Slog.i(ActivityManagerService.TAG, "Killing " + toShortString() + " (adj " + setAdj + "): " + reason);
			}
	...
	public boolean addPackage(String pkg, int versionCode, ProcessStatsService tracker) {
		if (!pkgList.containsKey(pkg)) {
			...
			if (baseProcessTracker != null) {
				holder.state = tracker.getProcessStateLocked(pkg, info.uid, versionCode, processName);
