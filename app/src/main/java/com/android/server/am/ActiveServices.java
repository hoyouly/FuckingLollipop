...
public final class ActiveServices {
     ...
     */
    final ArrayMap<IBinder, ArrayList<ConnectionRecord>> mServiceConnections = new ArrayMap<IBinder, ArrayList<ConnectionRecord>>();
     ...
     */
    final ArrayList<ServiceRecord> mDestroyingServices = new ArrayList<ServiceRecord>();
    ...
    class ServiceMap extends Handler {
        final int mUserId;
        final ArrayMap<ComponentName, ServiceRecord> mServicesByName = new ArrayMap<ComponentName, ServiceRecord>();
        final ArrayMap<Intent.FilterComparison, ServiceRecord> mServicesByIntent = new ArrayMap<Intent.FilterComparison, ServiceRecord>();

        final ArrayList<ServiceRecord> mDelayedStartList = new ArrayList<ServiceRecord>();
        ...
        */
        ...
        final ArrayList<ServiceRecord> mStartingBackground = new ArrayList<ServiceRecord>();
    ...
    public ActiveServices(ActivityManagerService service) {
        ...
        mMaxStartingBackground = maxBg > 0 ? maxBg : ActivityManager.isLowRamDeviceStatic() ? 1 : 8;
    }

    ServiceRecord getServiceByName(ComponentName name, int callingUser) {
        // TODO: Deal with global services
        if (DEBUG_MU) Slog.v(TAG_MU, "getServiceByName(" + name + "), callingUser = " + callingUser);
    ...
    ComponentName startServiceLocked(IApplicationThread caller, Intent service, String resolvedType, int callingPid, int callingUid, int userId) {
        if (DEBUG_DELAYED_STARTS) Slog.v(TAG, "startService: " + service + " type=" + resolvedType + " args=" + service.getExtras());
        ...
        if (!callerFg && r.app == null && mAm.mStartedUsers.get(r.userId) != null) {
            ...
            if (proc == null || proc.curProcState > ActivityManager.PROCESS_STATE_RECEIVER) {
                ...
                if (DEBUG_DELAYED_SERVICE) Slog.v(TAG, "Potential start delay of " + r + " in " + proc);
                if (r.delayed) {
            ...
            if (callerFg) {
                Slog.v(TAG, "Not potential delay (callerFg=" + callerFg + " uid=" + callingUid + " pid=" + callingPid + "): " + r);
            } else if (r.app != null) {
                ...
                if (b != null && !b.received) {
                    ...
                    for (int conni = r.connections.size() - 1; conni >= 0; conni--) {
                        ...
                        for (int i = 0; i < clist.size(); i++) {
                            ...
                            try {
                                // c的类型是ConnectionRecord  c.conn的类型就是ServiceDispatcher.InnerConnection
    ...
    private ServiceLookupResult retrieveServiceLocked(Intent service, String resolvedType, int callingPid, int callingUid, int userId, boolean createIfNeeded, boolean callingFromFg) {
        ...
        userId = mAm.handleIncomingUser(callingPid, callingUid, userId, false, ActivityManagerService.ALLOW_NON_FULL_IN_PROFILE, "service", null);
    ...
    private final void bumpServiceExecutingLocked(ServiceRecord r, boolean fg, String why) {
        if (DEBUG_SERVICE) Slog.v(TAG, ">>> EXECUTING " + why + " of " + r + " in app " + r.app);
        else if (DEBUG_SERVICE_EXECUTING) Slog.v(TAG, ">>> EXECUTING " + why + " of " + r.shortName);
    ...
    private final String bringUpServiceLocked(ServiceRecord r, int intentFlags, boolean execInFg, boolean whileRestarting) {
        ...
        if (app == null) {
            if ((app = mAm.startProcessLocked(procName, r.appInfo, true, //
                    intentFlags, "service", r.name, false, isolated, false)) == null) {
                String msg = "Unable to launch app " + r.appInfo.packageName + "/" + r.appInfo.uid //
                        + " for service " + r.intent.getIntent() + ": process is bad";
                Slog.w(TAG, msg);
                bringDownServiceLocked(r);
                return msg;
            }
    ...
    private void serviceDoneExecutingLocked(ServiceRecord r, boolean inDestroying, boolean finishing) {
        if (DEBUG_SERVICE)
            Slog.v(TAG, "<<< DONE EXECUTING " + r + ": nesting=" + r.executeNesting + ", inDestroying=" + inDestroying + ", app=" + r.app);
