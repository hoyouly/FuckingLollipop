...
public final class ActivityStackSupervisor implements DisplayListener {
     ...
     */
    private final SparseArray<ActivityDisplay> mActivityDisplays = new SparseArray<ActivityDisplay>();
    ...
    final ArrayList<PendingActivityLaunch> mPendingActivityLaunches = new ArrayList<PendingActivityLaunch>();
    ...
    private IDevicePolicyManager getDevicePolicyManager() {
        synchronized (mService) {
            if (mDevicePolicyManager == null) {
                mDevicePolicyManager = IDevicePolicyManager.Stub.asInterface(ServiceManager.checkService(Context.DEVICE_POLICY_SERVICE));
                if (mDevicePolicyManager == null) {
    ...
    void moveHomeStack(boolean toFront, String reason) {
        ...
        if (homeInFront != toFront) {
            ...
            if (DEBUG_STACK)
                Slog.d(TAG, "moveHomeTask: topStack old=" + topStack + " new=" + mFocusedStack);
        }
        ...
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            ...
            for (int stackNdx = stacks.size() - 1; stackNdx >= 0; --stackNdx) {
                ...
                if (resumedActivity == null || !resumedActivity.idle) {
                    if (DEBUG_STATES)
                        Slog.d(TAG, "allResumedActivitiesIdle: stack=" + stack.mStackId + " " + resumedActivity + " not idle");
                    return false;
                }
            }
        }
    ...
    boolean allResumedActivitiesComplete() {
        ...
        if (DEBUG_STACK)
            Slog.d(TAG, "allResumedActivitiesComplete: mLastFocusedStack changing from=" + mLastFocusedStack + " to=" + mFocusedStack);
     ...
     * 暂停所有的堆栈中的所有activities 或者只是后面的堆栈。
        ...
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            ...
            for (int stackNdx = stacks.size() - 1; stackNdx >= 0; --stackNdx) {
                ...
                if (!isFrontStack(stack) && stack.mResumedActivity != null) {
                    if (DEBUG_STATES)
                        Slog.d(TAG, "pauseBackStacks: stack=" + stack + " mResumedActivity=" + stack.mResumedActivity);
    ...
    final int startActivityMayWait(IApplicationThread caller, int callingUid, String callingPackage, Intent intent, String resolvedType, //
                                   IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor, IBinder resultTo, String resultWho,//
                                   int requestCode, int startFlags, ProfilerInfo profilerInfo, WaitResult outResult, Configuration config, Bundle options,//
        ...
        synchronized (mService) {
            ...
            if (DEBUG_CONFIGURATION)
                Slog.v(TAG, "Starting activity when config will change = " + stack.mConfigWillChange);
            ...
            if (aInfo != null && (aInfo.applicationInfo.flags & ApplicationInfo.FLAG_CANT_SAVE_STATE) != 0) {
                ...
                if (aInfo.processName.equals(aInfo.applicationInfo.packageName)) {
                    if (mService.mHeavyWeightProcess != null && (mService.mHeavyWeightProcess.info.uid != aInfo.applicationInfo.uid //
                        ...
                        IIntentSender target = mService.getIntentSenderLocked(ActivityManager.INTENT_SENDER_ACTIVITY, "android", appCallingUid, userId, //
                                null, null, 0, new Intent[]{intent}, new String[]{resolvedType}, //
                                PendingIntent.FLAG_CANCEL_CURRENT | PendingIntent.FLAG_ONE_SHOT, null);
            ...
            int res = startActivityLocked(caller, intent, resolvedType, aInfo, voiceSession, voiceInteractor, resultTo, resultWho, requestCode, //
                    callingPid, callingUid, callingPackage, realCallingPid, realCallingUid, startFlags, options, componentSpecified, null, container, inTask);
        ...
        try {
            synchronized (mService) {
                ...
                for (int i = 0; i < intents.length; i++) {
                    ...
                    int res = startActivityLocked(caller, intent, resolvedTypes[i], aInfo, null, null, resultTo, null, -1, callingPid, callingUid, callingPackage, callingPid, callingUid, 0, theseOptions, componentSpecified, outActivity, null, null);
                    if (res < 0) {
                        return res;
                    }
    ...
    final boolean realStartActivityLocked(ActivityRecord r, ProcessRecord app, boolean andResume, boolean checkConfig) throws RemoteException {
        ...
        try {
            ...
            ProfilerInfo profilerInfo = profileFile != null ? new ProfilerInfo(profileFile, profileFd, mService.mSamplingInterval, mService.mAutoStopProfiler) : null;
            app.forceProcessStateUpTo(ActivityManager.PROCESS_STATE_TOP);
            //app.thread 类型为IApplicatioThread,其实就是ApplicationThread
            app.thread.scheduleLaunchActivity(new Intent(r.intent), r.appToken, System.identityHashCode(r), r.info, new Configuration(mService.mConfiguration),//
                    r.compat, r.launchedFromPackage, r.task.voiceInteractor, app.repProcState, r.icicle, r.persistentState, //
    ...
    final int startActivityLocked(IApplicationThread caller, Intent intent, String resolvedType, ActivityInfo aInfo, IVoiceInteractionSession voiceSession, //
                                  IVoiceInteractor voiceInteractor, IBinder resultTo, String resultWho, int requestCode, int callingPid, int callingUid, //
                                  String callingPackage, int realCallingPid, int realCallingUid, int startFlags, Bundle options, boolean componentSpecified,//
        ...
        if (err == ActivityManager.START_SUCCESS) {
            ...
            Slog.i(TAG, "START u" + userId + " {" + intent.toShortString(true, true, true, false) + "} from uid " + callingUid + " on display " + (container == null ? (mFocusedStack == null ? Display.DEFAULT_DISPLAY : mFocusedStack.mDisplayId) : (container.mActivityDisplay == null ? Display.DEFAULT_DISPLAY : container.mActivityDisplay.mDisplayId)));
        }
        ...
        ActivityRecord r = new ActivityRecord(mService, callerApp, callingUid, callingPackage, intent, resolvedType, aInfo, mService.mConfiguration,//
                resultRecord, resultWho, requestCode, componentSpecified, this, container, options);
        if (outActivity != null) {
            outActivity[0] = r;
        }
        ...
        //启动Activity  startActivityUnchecked方法负责调度ActivityRecord和Task，理解该方法是理解Actvity启动模式的关键。
    ...
    ActivityStack adjustStackFocus(ActivityRecord r, boolean newTask) {
        ...
        if (!mLeanbackOnlyDevice && (r.isApplicationActivity() || (task != null && task.isApplicationTask()))) {
                ...
                if (taskStack.isOnHomeDisplay()) {
                    if (mFocusedStack != taskStack) {
                        if (DEBUG_FOCUS || DEBUG_STACK)
                            Slog.d(TAG, "adjustStackFocus: Setting " + "focused stack to r=" + r + " task=" + task);
                        mFocusedStack = taskStack;
                    } else {
                        if (DEBUG_FOCUS || DEBUG_STACK)
                            Slog.d(TAG, "adjustStackFocus: Focused stack already=" + mFocusedStack);
                    }
                }
                return taskStack;
            }
            ...
            if (mFocusedStack != mHomeStack && (!newTask || mFocusedStack.mActivityContainer.isEligibleForNewTasks())) {
                if (DEBUG_FOCUS || DEBUG_STACK)
                    Slog.d(TAG, "adjustStackFocus: Have a focused stack=" + mFocusedStack);
                return mFocusedStack;
            }
    ...
    final int startActivityUncheckedLocked(ActivityRecord r, ActivityRecord sourceRecord, IVoiceInteractionSession voiceSession, IVoiceInteractor voiceInteractor, //
        ...
        if (sourceRecord == null && inTask != null && inTask.stack != null) {
            ...
            if (root == null) {
                final int flagsOfInterest = Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_MULTIPLE_TASK | Intent.FLAG_ACTIVITY_NEW_DOCUMENT | Intent.FLAG_ACTIVITY_RETAIN_IN_RECENTS;
        ...
        if (((launchFlags & Intent.FLAG_ACTIVITY_NEW_TASK) != 0 && (launchFlags & Intent.FLAG_ACTIVITY_MULTIPLE_TASK) == 0) || launchSingleInstance || launchSingleTask) {
            ...
            if (inTask == null && r.resultTo == null) {
                ...
                if (intentActivity != null) {
                    ...
                    if (DEBUG_TASKS)
                        Slog.d(TAG, "Bring to front target: " + targetStack + " from " + intentActivity);
                    ...
                    } else if ((launchFlags & Intent.FLAG_ACTIVITY_CLEAR_TOP) != 0 || launchSingleInstance || launchSingleTask) {
        ...
        TaskRecord taskToAffiliate = launchTaskBehind && sourceRecord != null ? sourceRecord.task : null;
        ...
        if (r.resultTo == null && inTask == null && !addingToTask && (launchFlags & Intent.FLAG_ACTIVITY_NEW_TASK) != 0) {
            ...
            if (DEBUG_TASKS)
                Slog.v(TAG, "Starting new activity " + r + " in existing task " + r.task + " from source " + sourceRecord);
            ...
            if (DEBUG_TASKS)
                Slog.v(TAG, "Starting new activity " + r + " in explicit task " + r.task);
            ...
            if (DEBUG_TASKS)
                Slog.v(TAG, "Starting new activity " + r + " in new guessed " + r.task);
        }
    ...
    final ActivityRecord activityIdleInternalLocked(final IBinder token, boolean fromTimeout, Configuration config) {
        ...
        if (r != null) {
            if (DEBUG_IDLE)
                Slog.d(TAG, "activityIdleInternalLocked: Callers=" + Debug.getCallers(4));
    ...
    void findTaskToMoveToFrontLocked(TaskRecord task, int flags, Bundle options, String reason) {
        ...
        if (DEBUG_STACK)
            Slog.d(TAG, "findTaskToMoveToFront: moved to front of stack=" + task.stack);
    }
    ...
    void removeChildActivityContainers(ActivityRecord parentActivity) {
        ...
        for (int containerNdx = childStacks.size() - 1; containerNdx >= 0; --containerNdx) {
            ...
            if (DEBUG_CONTAINERS)
                Slog.d(TAG, "removeChildActivityContainers: removing " + container);
            container.release();
        }
    }
    ...
    void deleteActivityContainer(IActivityContainer container) {
        ...
        if (activityContainer != null) {
            if (DEBUG_CONTAINERS)
                Slog.d(TAG, "deleteActivityContainer: ", new RuntimeException("here").fillInStackTrace());
        ...
        for (int displayNdx = mActivityDisplays.size() - 1; displayNdx >= 0; --displayNdx) {
            ...
            for (int stackNdx = stacks.size() - 1; stackNdx >= 0; --stackNdx) {
                ...
                if (!stack.mActivityContainer.isEligibleForNewTasks()) {
                    if (DEBUG_TASKS) Slog.d(TAG, "Skipping stack: (new task not allowed) " + stack);
                    continue;
                }
    ...
    void checkReadyForSleepLocked() {
        ...
        if (!mSleepTimeout) {
            ...
            if (mStoppingActivities.size() > 0) {
                ...
                if (DEBUG_PAUSE)
                    Slog.v(TAG, "Sleep still need to stop " + mStoppingActivities.size() + " activities");
                scheduleIdleLocked();
                dontSleep = true;
            }
            ...
            if (mGoingToSleepActivities.size() > 0) {
                ...
                if (DEBUG_PAUSE)
                    Slog.v(TAG, "Sleep still need to sleep " + mGoingToSleepActivities.size() + " activities");
                dontSleep = true;
            }
    ...
    boolean requestVisibleBehindLocked(ActivityRecord r, boolean visible) {
        ...
        if (stack == null) {
            if (DEBUG_VISIBLE_BEHIND)
                Slog.d(TAG, "requestVisibleBehind: r=" + r + " visible=" + visible + " stack is null");
            return false;
        }
        final boolean isVisible = stack.hasVisibleBehindActivity();
        if (DEBUG_VISIBLE_BEHIND)
            Slog.d(TAG, "requestVisibleBehind r=" + r + " visible=" + visible + " isVisible=" + isVisible);
        ...
        if (visible && top.fullscreen) {
            ...
            if (DEBUG_VISIBLE_BEHIND)
                Slog.d(TAG, "requestVisibleBehind: returning top.fullscreen=" + top.fullscreen + " top.state=" + top.state + " top.app=" + top.app + " top.app.thread=" + top.app.thread);
            return false;
            ...
            if (DEBUG_VISIBLE_BEHIND)
                Slog.d(TAG, "requestVisibleBehind: returning visible=" + visible + " stack.getVisibleBehindActivity()=" + stack.getVisibleBehindActivity() + " r=" + r);
            return false;
        }
...
*/
    ...
    class ActivityContainer extends android.app.IActivityContainer.Stub {
        final static int FORCE_NEW_TASK_FLAGS = Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_MULTIPLE_TASK | Intent.FLAG_ACTIVITY_NO_ANIMATION;
        final int mStackId;
        ...
        void attachToDisplayLocked(ActivityDisplay activityDisplay) {
            if (DEBUG_STACK)
                Slog.d(TAG, "attachToDisplayLocked: " + this + " to display=" + activityDisplay);
        ...
        protected void detachLocked() {
            if (DEBUG_STACK)
                Slog.d(TAG, "detachLocked: " + this + " from display=" + mActivityDisplay + " Callers=" + Debug.getCallers(2));
            if (mActivityDisplay != null) {
        ...
        void setVisible(boolean visible) {
            if (mVisible != visible) {
                mVisible = visible;
                if (mCallback != null) {
                    mHandler.obtainMessage(CONTAINER_CALLBACK_VISIBILITY, visible ? 1 : 0, 0 /* unused */, this).sendToTarget();
                }
            }
        }
    ...
    private class VirtualActivityContainer extends ActivityContainer {
        ...
        private void setSurfaceLocked(Surface surface, int width, int height, int density) {
            ...
            if (DEBUG_STACK)
                Slog.d(TAG, "setSurface: " + this + " to display=" + virtualActivityDisplay);
        }
        ...
        private void setSurfaceIfReadyLocked() {
            if (DEBUG_STACK)
                Slog.v(TAG, "setSurfaceIfReadyLocked: mDrawn=" + mDrawn + " mContainerState=" + mContainerState + " mSurface=" + mSurface);
    ...
    class ActivityDisplay {
        ...
        void attachActivities(ActivityStack stack) {
            if (DEBUG_STACK)
                Slog.v(TAG, "attachActivities: attaching " + stack + " to displayId=" + mDisplayId);
            mStacks.add(stack);
        }

        void detachActivitiesLocked(ActivityStack stack) {
            if (DEBUG_STACK)
                Slog.v(TAG, "detachActivitiesLocked: detaching " + stack + " from displayId=" + mDisplayId);
    ...
    class VirtualActivityDisplay extends ActivityDisplay {
        ...
        VirtualActivityDisplay(int width, int height, int density) {
            ...
            mVirtualDisplay = dm.createVirtualDisplay(mService.mContext, null, VIRTUAL_DISPLAY_BASE_NAME, width, height, density, null, DisplayManager.VIRTUAL_DISPLAY_FLAG_PUBLIC | DisplayManager.VIRTUAL_DISPLAY_FLAG_OWN_CONTENT_ONLY, null, null);
    ...
    private boolean isLeanbackOnlyDevice() {
        ...
        try {
            onLeanbackOnly = AppGlobals.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LEANBACK_ONLY);
        } catch (RemoteException e) {
            // noop
        }
