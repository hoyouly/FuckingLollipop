...
final class ActivityRecord {
	...
	void setTaskToAffiliateWith(TaskRecord taskToAffiliateWith) {
		if (taskToAffiliateWith != null && launchMode != ActivityInfo.LAUNCH_SINGLE_INSTANCE && launchMode != ActivityInfo.LAUNCH_SINGLE_TASK) {
	...
	boolean isPersistable() {
		return (info.persistableMode == ActivityInfo.PERSIST_ROOT_ONLY || info.persistableMode == ActivityInfo.PERSIST_ACROSS_REBOOTS) && (intent == null || (intent.getFlags() & Intent.FLAG_ACTIVITY_EXCLUDE_FROM_RECENTS) == 0);
	}
	...
	final void deliverNewIntentLocked(int callingUid, Intent intent, String referrer) {
		...
		boolean unsent = true;
		if ((state == ActivityState.RESUMED || (service.isSleeping() && task.stack.topRunningActivityLocked(null) == this)) && app != null && app.thread != null) {
			try {
	...
	void updateThumbnailLocked(Bitmap newThumbnail, CharSequence description) {
		if (newThumbnail != null) {
			if (ActivityManagerService.DEBUG_THUMBNAILS) Slog.i(ActivityManagerService.TAG, "Setting thumbnail of " + this + " to " + newThumbnail);
	...
	public void windowsGone() {
		if (ActivityManagerService.DEBUG_SWITCH) Log.v(ActivityManagerService.TAG, "windowsGone(): " + this);
		nowVisible = false;
	}
	...
	public boolean isInterestingToUserLocked() {
		return visible || nowVisible || state == ActivityState.PAUSING || state == ActivityState.RESUMED;
	}
	...
	private static String createImageFilename(long createTime, int taskId) {
		return String.valueOf(taskId) + ACTIVITY_ICON_SUFFIX + createTime + TaskPersister.IMAGE_EXTENSION;
	}
	...
	static ActivityRecord restoreFromXml(XmlPullParser in, ActivityStackSupervisor stackSupervisor) throws IOException, XmlPullParserException {
		...
		for (int attrNdx = in.getAttributeCount() - 1; attrNdx >= 0; --attrNdx) {
			...
			if (DEBUG_PERSISTER || DEBUG_RESTORER) Slog.d(TaskPersister.TAG, "ActivityRecord: attribute name=" + attrName + " value=" + attrValue);
		...
		while (((event = in.next()) != XmlPullParser.END_DOCUMENT) && (event != XmlPullParser.END_TAG || in.getDepth() < outerDepth)) {
			if (event == XmlPullParser.START_TAG) {
				...
				if (DEBUG_PERSISTER || DEBUG_RESTORER) Slog.d(TaskPersister.TAG, "ActivityRecord: START_TAG name=" + name);
				if (TAG_INTENT.equals(name)) {
					intent = Intent.restoreFromXml(in);
					if (DEBUG_PERSISTER || DEBUG_RESTORER) Slog.d(TaskPersister.TAG, "ActivityRecord: intent=" + intent);
				} else if (TAG_PERSISTABLEBUNDLE.equals(name)) {
					persistentState = PersistableBundle.restoreFromXml(in);
					if (DEBUG_PERSISTER || DEBUG_RESTORER) Slog.d(TaskPersister.TAG, "ActivityRecord: persistentState=" + persistentState);
				} else {
		...
		final ActivityRecord r = new ActivityRecord(service, /*caller*/null, launchedFromUid, launchedFromPackage, intent, resolvedType, aInfo, service.getConfiguration(), null, null, 0, componentSpecified, stackSupervisor, null, null);
